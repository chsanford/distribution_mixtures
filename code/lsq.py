...
gamma = 0.9
mdp = rmdp.RandomMDP(num_states, num_actions, gamma=gamma, seed=12)
opt_pol = mdp.p_opt[0]
...
def main():
    ...
    target_pol = ''.join([str(np.random.choice(num_actions)) for _ in range(num_states)])

    print "Using policies: ",
    print left_pol,
    print right_pol,
    print "(target)", target_pol
    ...
    q_e = estimate_q(left_sample, target_pol)
    get_weighted_residual(q_e, target_pol, left_dist)

    """
    print "Opt pol: ", opt_pol
    print "get_error(left_sample, left_dist, opt_pol)"
    left_errs = np.asarray(get_error(estimate_q(left_sample, opt_pol), left_dist))
    print "get_error(left_sample, right_dist, opt_pol)"
    right_errs = np.asarray(get_error(estimate_q(left_sample, opt_pol), right_dist))

    print "get_error(right_sample, left_dist, opt_pol)"
    left_errs = np.append(left_errs, get_error(estimate_q(right_sample, opt_pol), left_dist), axis=0)
    print "get_error(right_sample, right_dist, opt_pol)"
    right_errs = np.append(right_errs, get_error(estimate_q(right_sample, opt_pol), right_dist), axis=0)
    ...
    print "On mix: "
    left_errs = np.append(left_errs, get_error(estimate_q(left_sample, opt_pol), mix_dist), axis=0)
    right_errs = np.append(right_errs, get_error(estimate_q(right_sample, opt_pol), mix_dist), axis=0)
    print "mix_wt: ", wt
...
def get_weighted_residual(q_est, target_pol, dist):

    p = [int (each) for each in target_pol]
    pi = np.zeros((num_states, num_actions))
    pi[range(len(p)), p] = 1.0
    pi = pi.reshape()

    err = 0.0
    for s, a in [(state, action) for state in range(num_states) for action in range(num_actions)]:
        # r = target_pol * transitions * rewards
        r =
        next_step =
        err += (q_est(s, a) - next_step) ** 2


    return err


def estimate_q(sample, target_pol):
    # Given a sample, estimate Q*
    A = np.zeros((num_features, num_features))
    b = np.zeros((num_features, 1))
    for (s, a, r, sp) in sample:
        A += np.dot(phi(s, a), np.transpose(phi(s, a) - gamma * phi(sp, int(target_pol[sp]))))
        b += phi(s, a) * r
    w = np.dot(np.linalg.inv(A), b)
    return lambda state, action: np.dot(np.transpose(phi(state, action)), w)


def get_error(q_est, dist):
