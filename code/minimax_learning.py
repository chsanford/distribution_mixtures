...
def minimax_learning(random_MDP, policy1, policy2, iterations):
    DISCOUNT_FACTOR = random_MDP.gamma
    NUMBER_STATES = random_MDP.num_states
    REWARD = random_MDP.rewards
    TRANSITION = random_MDP.transitions
    assert random_MDP.num_actions == 2
    ...
    dist1 = get_state_action_dist(np.asarray(policy1))
    dist2 = get_state_action_dist(np.asarray(policy2))
    print(dist1)
    print(dist2)
    iteratively_learn_distributions(iterations, dist1, dist2)

rmdp = rm.RandomMDP(4, 2)
print(rmdp.transitions)
print(rmdp.rewards)
minimax_learning(rmdp, [0, 0, 0, 0], [1, 1, 1, 1], 5)
